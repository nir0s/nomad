...
func (tc *ConsulTemplateTest) TestUpdatesRestartTasks(f *framework.F) {
	...
	for _, event := range events {
		if event.Type == "Template" && strings.Contains(event.DisplayMessage, "kv.block") {
			found = true
		}
	}
	require.True(found, "Could not find template block event")
	...
	// FIXME: This restart counter should only be 1
