...
func (tc *TaskEventsTest) waitUntilEvents(f *framework.F, job string, numEvents int) *api.TaskState {
	t := f.T()
	nomadClient := tc.Nomad()
	uuid := uuid.Generate()
	jobId := "te_" + uuid[0:8]
	...
	jobFile := fmt.Sprintf("taskevents/input/%s.nomad", job)
	allocs := e2eutil.RegisterAndWaitForAllocs(f, nomadClient, jobFile, job)
	...
	testutil.WaitForResultRetries(10, func() (bool, error) {
		...
		// Capture task state
		taskState = a.TaskStates[job]
		if taskState == nil {
			return false, fmt.Errorf("task state not found for %s", job)
		}
